...
import { paiService } from "../services/api";
...
export default function CadastroPai() {
  ...
  const lidarComEnvio = async (evento) => {
    ...
    try {
      await paiService.criar(formulario);
  ...
  return (
    <div className="flex justify-center items-center min-h-[70vh]">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl p-8">
        <h2 className="text-2xl font-bold text-blue-900 mb-6 text-center">Cadastro de Responsável</h2>
        {mensagem && (
        ...
        <form onSubmit={lidarComEnvio} className="space-y-5">
          ...
          <div>
            <input
              ...
              onChange={(e) => lidarComMudanca("endereco", e.target.value)}
              className="w-full border border-blue-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 text-blue-900 bg-blue-50"
              required
            />
          </div>
